...
class Player(object):
    ...
    # Redefine mplayer.py _run_command method to identify the file on load
    def _run_command(self, name, *args):
        ...
        # Expect a response for loadfile with -inspect option
        if '-identify' in self.args and name == 'loadfile' and self._proc.stdout is not None:
            # The reponses for properties start with 'ID_<property name>'
            keys = ['ID_VIDEO_ID=', 'ID_AUDIO_ID=', 'ID_SUBTITLE_ID=', 'ID_VID_', 'ID_AID_', 'ID_SID_']
            info = {'video':{}, 'audio':{}, 'subtitle':{}}
            self.info = info
            while True:
                try:
                    res = self._stdout._identificators.get(timeout=1.0)
                except queue.Empty:
                    return
                if res.startswith('ID_FILENAME'):
                    break
                for key in keys:
                    if res.startswith(key):
                        ans = str(res.partition('=')[2].strip('\'"'))
                        if key == keys[0]:
                            info['video'][ans] = {}
                        if key == keys[1]:
                            info['audio'][ans] = {}
                        if key == keys[2]:
                            info['subtitle'][ans] = {}
                        if key == keys[3]:
                            ans = str(res.replace('_', ' '))
                            ans = ans.replace('=', ' ')
                            ans = ans.split(' ')
                            info['video'][ans[2]][ans[3].lower()] = ans[4]
                        if key == keys[4]:
                            ans = str(res.replace('_', ' '))
                            ans = ans.replace('=', ' ')
                            ans = ans.split(' ')
                            info['audio'][ans[2]][ans[3].lower()] = ans[4]
                        if key == keys[5]:
                            ans = str(res.replace('_', ' '))
                            ans = ans.replace('=', ' ')
                            ans = ans.split(' ')
                            info['subtitle'][ans[2]][ans[3].lower()] = ans[4]
            return

        # Expect a response for 'get_property'
